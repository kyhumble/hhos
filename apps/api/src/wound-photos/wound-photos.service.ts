/**
 * Wound photo control plane (PR 5b + PR 6 + PR 7).
 ...
 * PR 7: annotations (child DEK, online-only), measurements PATCH, large-wound task hook (K9/K27/K29).
 * Task row creation is delegated to ClinicalTasksService (sole owner).
 */
import {
  BadRequestException,
  ConflictException,
...
export type PhotoAnnotationRow = typeof photoAnnotations.$inferSelect;
...
const ANNOTATION_CONTENT_TYPE: Record<string, string> = {
...
function safeAnnotationAudit(row: PhotoAnnotationRow | null | undefined) {
  if (!row) return null;
  return {
    id: row.id,
    orgId: row.orgId,
    woundPhotoId: row.woundPhotoId,
    clientAnnotationId: row.clientAnnotationId,
...
function toAnnotationMetadata(row: PhotoAnnotationRow) {
  return {
    id: row.id,
    orgId: row.orgId,
    woundPhotoId: row.woundPhotoId,
    ...
    annotationType: row.annotationType,
    status: row.status,
    contentType: ANNOTATION_CONTENT_TYPE[row.annotationType] ?? 'application/octet-stream',
    byteSize: row.byteSize ?? null,
    cipherSha256: row.cipherSha256 ?? null,
    hasWrappedDek: Boolean(row.wrappedDek),
    kekKeyId: row.kekKeyId ?? null,
...
export class WoundPhotosService {
  ...
  async complete(
    ...
    // K29: ClinicalTasksService is sole owner of clinical_tasks rows
    if (updated.isLargeWound) {
      await this.clinicalTasks.ensureOpenLargeWoundReview(updated, {
        actorUserId: user.id,
        meta,
      });
    }
  ...
  // ─── Measurements PATCH (PR 7) ────────────────────────────────────────────

  /**
   * PATCH /v1/wound-photos/:id/measurements
   * Available photos only. Recomputes is_large_wound; creates large_wound_review
   * when newly large. Never auto-cancels open tasks if measurements fall below threshold.
   */
  async patchMeasurements(
    ...
    if (photo.status !== 'available') {
      throw new ConflictException({
        error: {
          code: 'INVALID_PHOTO_STATE',
          message: `Measurements can only be patched when status is available (got ${photo.status})`,
        },
      });
    }
    ...
    const lengthCm =
      input.lengthCm !== undefined
        ? input.lengthCm
        : parseNumericCm(photo.lengthCm);
    const widthCm =
      input.widthCm !== undefined
        ? input.widthCm
        : parseNumericCm(photo.widthCm);
    const depthCm =
      input.depthCm !== undefined
        ? input.depthCm
        : parseNumericCm(photo.depthCm);

    const orgSettings = await this.loadOrgSettings(user.orgId);
    const isLarge = computeIsLargeWound(lengthCm, widthCm, orgSettings);
    ...
    if (!updated) {
      throw new ConflictException({
        error: {
          code: 'INVALID_PHOTO_STATE',
          message: 'Photo could not be updated (state changed)',
        },
      });
    }

    // Create task when large; never cancel if no longer large (K29 / design)
    if (updated.isLargeWound) {
      await this.clinicalTasks.ensureOpenLargeWoundReview(updated, {
        actorUserId: user.id,
        meta,
    ...
    await this.audit.writeFromUser(user, {
      action: 'wound_photo.measurements_update',
      resourceType: 'wound_photo',
      resourceId: updated.id,
      patientId: updated.patientId,
      episodeId: updated.episodeId,
  ...
  // ─── Annotations (child DEK, online-only — K9/K27) ────────────────────────

  /**
   * POST /v1/wound-photos/:id/annotations/uploads
   * Parent must be available. Own child DEK (never parent DEK). No payload_json.
   */
  async initiateAnnotation(
    user: AuthUser,
    photoId: string,
    ...
    if (
      meta?.idempotencyKey &&
      meta.idempotencyKey !== input.clientAnnotationId
    ) {
      throw new BadRequestException({
        error: {
          code: 'VALIDATION_FAILED',
          message: 'Idempotency-Key must equal clientAnnotationId',
        },
      });
    }
    ...
    if (photo.status !== 'available') {
      throw new ConflictException({
        error: {
          ...
          message: `Annotation requires parent photo status available (got ${photo.status})`,
        },
      });
    }
    ...
    const existing = await this.findAnnotationByClientId(
      user.orgId,
      input.clientAnnotationId,
    );
    if (existing) {
      return this.replayOrConflictAnnotationInitiate(
        user,
        existing,
        photo,
        input,
    ...
    const storageKey = this.storage.woundPhotoAnnotationObjectKey(
      user.orgId,
      photo.id,
      annotationId,
    );
    ...
    try {
      const row = await this.db.transaction(async (tx) => {
        ...
        if (!created) {
          throw new Error('photo_annotations insert returned no row');
        }

        await this.audit.writeFromUser(
          user,
          {
            action: 'wound_photo.annotation_initiate',
            resourceType: 'photo_annotation',
            resourceId: created.id,
            patientId: photo.patientId,
            episodeId: photo.episodeId,
            after: safeAnnotationAudit(created),
            requestId: meta?.requestId,
            ip: meta?.ip,
            userAgent: meta?.userAgent,
            deviceId: input.device.deviceId,
      ...
      const presign = await this.presignForAnnotation(row);
      return this.annotationInitiateResponse(row, presign);
    } catch (err) {
      if (isUniqueViolation(err)) {
        const raced = await this.findAnnotationByClientId(
          user.orgId,
          input.clientAnnotationId,
        );
        if (raced) {
  ...
  /**
   * POST /v1/annotations/:id/wrap-dek
   * Child DEK single-use wrap. Device gate uses parent photo deviceId.
   */
  async wrapAnnotationDek(
    user: AuthUser,
    annotationId: string,
    input: WrapDekInput,
    meta?: RequestMeta,
    ...
    const annotation = await this.loadAnnotationOrThrow(
      user.orgId,
      annotationId,
    );
    const photo = await this.loadPhotoOrThrow(
      user.orgId,
      annotation.woundPhotoId,
    );
    await this.assertEpisodeAccess(user, photo.episodeId);
    await this.devices.assertActiveDevice(user.orgId, photo.deviceId);
    ...
    if (photo.status !== 'available') {
      throw new ConflictException({
        error: {
          code: 'PARENT_NOT_AVAILABLE',
          message: 'Parent photo must be available for annotation wrap',
        },
      });
    }

    if (annotation.wrappedDek || annotation.status !== 'pending_upload') {
      throw new ConflictException({
        error: {
          code: 'DEK_ALREADY_WRAPPED',
          message: 'DEK has already been wrapped for this annotation',
        },
      });
    }
    ...
    try {
      ...
      const [updated] = await this.db
        ...
        .where(
          and(
            eq(photoAnnotations.id, annotation.id),
            eq(photoAnnotations.orgId, user.orgId),
            eq(photoAnnotations.status, 'pending_upload'),
            isNull(photoAnnotations.wrappedDek),
          ),
        )
        .returning();
      ...
      await this.audit.writeFromUser(user, {
        action: 'wound_photo.annotation_dek_wrapped',
        resourceType: 'photo_annotation',
        resourceId: updated.id,
        patientId: photo.patientId,
        episodeId: photo.episodeId,
        before: safeAnnotationAudit(annotation),
        after: safeAnnotationAudit(updated),
        requestId: meta?.requestId,
        ip: meta?.ip,
        userAgent: meta?.userAgent,
      ...
      return {
        id: updated.id,
        clientAnnotationId: updated.clientAnnotationId,
        status: updated.status,
        kekKeyId: updated.kekKeyId,
      };
    } finally {
   ...
   * Full ciphertext hash via internal S3; own child DEK envelope.
   */
  async completeAnnotation(
    ...
    if (annotation.clientAnnotationId !== input.clientAnnotationId) {
      throw new BadRequestException({
        error: {
          code: 'VALIDATION_FAILED',
          message: 'clientAnnotationId does not match annotation',
        },
      });
    }

    if (
      annotation.status === 'soft_deleted' ||
      annotation.status === 'abandoned'
    ) {
      throw new GoneException({
        error: {
          code: 'ANNOTATION_GONE',
          message: `Annotation is ${annotation.status}`,
        },
      });
    }
    ...
    if (annotation.status === 'available') {
      if (
        annotation.cipherSha256?.toLowerCase() === cipherSha &&
        annotation.byteSize === input.byteSize
      ) {
        return {
          id: annotation.id,
          clientAnnotationId: annotation.clientAnnotationId,
      ...
      throw new ConflictException({
        error: {
          code: 'INTEGRITY_MISMATCH',
          message:
            'Annotation already available with different cipher hash/size',
        },
      });
    }

    if (annotation.status !== 'pending_put' || !annotation.wrappedDek) {
      throw new ConflictException({
        error: {
          code: 'INVALID_ANNOTATION_STATE',
          message: `Annotation status must be pending_put with wrapped DEK (got ${annotation.status})`,
        },
      });
    }
    ...
    if (photo.status !== 'available') {
      throw new ConflictException({
        error: {
          code: 'PARENT_NOT_AVAILABLE',
          message: 'Parent photo must be available for annotation complete',
        },
      });
    }
    ...
    if (!annotation.storageKey) {
      throw new ServiceUnavailableException({
        error: {
          code: 'STORAGE_KEY_MISSING',
          message: 'Annotation has no storage key',
        },
      });
    }
    ...
    if (digestHex !== cipherSha || byteLength !== input.byteSize) {
      this.logger.warn(
        `annotation integrity mismatch annotationId=${annotation.id}`,
      );
      throw new ConflictException({
        error: {
          code: 'INTEGRITY_MISMATCH',
          message:
            'Ciphertext SHA-256 or byteSize does not match stored object',
        },
      });
    }
    ...
    const [updated] = await this.db
      ...
      .where(
        and(
          eq(photoAnnotations.id, annotation.id),
          eq(photoAnnotations.orgId, user.orgId),
          eq(photoAnnotations.status, 'pending_put'),
        ),
      )
      .returning();

    if (!updated) {
      const again = await this.loadAnnotationOrThrow(user.orgId, annotationId);
      if (
        again.status === 'available' &&
        again.cipherSha256?.toLowerCase() === cipherSha
      ) {
        return {
          id: again.id,
          clientAnnotationId: again.clientAnnotationId,
          status: again.status,
        };
      }
      throw new ConflictException({
        error: {
          code: 'INVALID_ANNOTATION_STATE',
          message: 'Annotation state changed during complete',
        },
      });
    }

    await this.audit.writeFromUser(user, {
      action: 'wound_photo.annotation_complete',
      resourceType: 'photo_annotation',
      resourceId: updated.id,
      patientId: photo.patientId,
      episodeId: photo.episodeId,
  ...
  /**
   * GET /v1/wound-photos/:id/annotations — metadata only (no ciphertext / DEK).
   */
  async listAnnotations(user: AuthUser, photoId: string) {
    this.assertFeatureEnabled();
    ...
    const rows = await this.db
      ...
      .where(
        and(
          eq(photoAnnotations.orgId, user.orgId),
          eq(photoAnnotations.woundPhotoId, photoId),
          ne(photoAnnotations.status, 'soft_deleted'),
          isNull(photoAnnotations.deletedAt),
        ),
      )
      .orderBy(desc(photoAnnotations.createdAt))
      .limit(200);

    return { data: rows.map(toAnnotationMetadata) };
  }

  /**
   * GET /v1/annotations/:id/content — decrypt proxy for child-DEK annotation.
   */
  async getAnnotationContent(
    ...
    if (annotation.status !== 'available') {
      throw new ConflictException({
        error: {
          code: 'INVALID_ANNOTATION_STATE',
          message: `Annotation content not available (status=${annotation.status})`,
        },
      });
    }

    if (!annotation.wrappedDek || !annotation.storageKey) {
      throw new ServiceUnavailableException({
        error: {
          code: 'ANNOTATION_ENVELOPE_INCOMPLETE',
          message: 'Annotation is missing wrapped DEK or storage key',
        },
      });
    }
    ...
    if (breakGlassRequested) {
      ...
      throw new ForbiddenException({
        error: {
          ...
          message:
            'Annotation content requires clinical purpose path or break-glass with reason',
        },
      });
    }
    ...
    try {
      // Child DEK — independent of parent photo DEK (K9)
      dek = this.photoCrypto.unwrapDek(
        annotation.wrappedDek,
        annotation.kekKeyId,
      );
      const cipherStream = await this.storage.getObjectStream(
        annotation.storageKey,
      );
      ...
      cipherStream.on('error', (err) => {
        this.logger.warn(
          `annotation content cipher stream error annotationId=${annotation.id}`,
        );
      ...
      if (usedBreakGlass) {
        await this.audit.write({
          ...
          action: 'wound_photo.annotation_view_break_glass',
          resourceType: 'photo_annotation',
        ...
        await this.audit.writeFromUser(user, {
          action: 'wound_photo.annotation_view',
          resourceType: 'photo_annotation',
          resourceId: annotation.id,
          patientId: photo.patientId,
          episodeId: photo.episodeId,
          after: safeAnnotationAudit(annotation),
          requestId: opts?.requestId,
          ip: opts?.ip,
          userAgent: opts?.userAgent,
          deviceId: photo.deviceId,
      ...
      return {
        ...
        contentType:
          ANNOTATION_CONTENT_TYPE[annotation.annotationType] ??
  ...
  private async loadAnnotationOrThrow(
    orgId: string,
    annotationId: string,
    ...
    const [row] = await this.db
      ...
      .where(
        and(
          eq(photoAnnotations.orgId, orgId),
          eq(photoAnnotations.id, annotationId),
        ),
      )
      .limit(1);
  ...
  private async findAnnotationByClientId(
    orgId: string,
    clientAnnotationId: string,
  ): Promise<PhotoAnnotationRow | null> {
    const [row] = await this.db
      .select()
      .from(photoAnnotations)
      .where(
        and(
          eq(photoAnnotations.orgId, orgId),
          eq(photoAnnotations.clientAnnotationId, clientAnnotationId),
        ),
      )
      .limit(1);
    return row ?? null;
  ...
  private async replayOrConflictAnnotationInitiate(
    user: AuthUser,
    existing: PhotoAnnotationRow,
    photo: WoundPhotoRow,
    input: InitiateAnnotationUploadInput,
  ) {
    if (
      existing.woundPhotoId !== photo.id ||
      ...
      throw new ConflictException({
        error: {
          code: 'CONFLICT',
          message:
            'clientAnnotationId already used with different photo/type',
        },
      });
    }
    ...
    if (PENDING_STATUSES.has(existing.status)) {
      await this.devices.assertActiveDevice(user.orgId, input.device.deviceId);
      const presign = await this.presignForAnnotation(existing);
      return this.annotationInitiateResponse(existing, presign, true);
    }

    return {
      id: existing.id,
      clientAnnotationId: existing.clientAnnotationId,
      status: existing.status,
      storageKey: existing.storageKey,
      _idempotentReplay: true as const,
    };
  ...
  private async presignForAnnotation(row: PhotoAnnotationRow) {
    if (!row.storageKey) {
      throw new ServiceUnavailableException({
        error: {
          code: 'STORAGE_KEY_MISSING',
  ...
  private annotationInitiateResponse(
    row: PhotoAnnotationRow,
    presign: { url: string; expiresAt: Date; key: string },
    replay = false,
  ) {
    return {
